...
class  CypherDisplayElementManager {
    ...
    createDisplayElementWithComponent(brandInfo, ownerInfo = null, isLoading = false) {

        // Fetch the message and icon for this combo of brand infor, owner info and loading state.
        const {message, icon} = this.createMessageAndIcon(brandInfo, ownerInfo, isLoading)
    ...
    insertDisplayElement(element) {
        ...
        if (this.layoutMode === 'product-details') {
            ...
            if (this.insertUnderPrice(element)) {
                // Don't return early, let it fall through to set this.displayElement
            } else {
        ...
        // Always set the display element
